...
namespace Rivet {
  ...
  public:
    ...
    void analyze(const Event& event) {
      ...
      if (isGoodEvent) {
        ...
        // Check that both jets are not overlapped
        ...
        foreach (const Jet* bJet, b_jets) {
          ...
          foreach (const Particle& p, bJet->particles()) {
            ...
            for (size_t i = 0; i <= idR; ++i) rings[i] += p.momentum().pT();
          }
          // Work out jet pT bin
          ...
          // Fill each dR bin of histos for this jet pT
  ...
  private:
    ...
    AIDA::IProfile1D* _p_b_rho[5];
    AIDA::IProfile1D* _p_l_rho[5];
    AIDA::IProfile1D* _p_b_Psi[5];
    AIDA::IProfile1D* _p_l_Psi[5];
