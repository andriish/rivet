...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      // Book histograms
      if (beamEnergyMatch(3.1*GeV)) { ///< @todo Was 1e-1 tolerance
        book(_h_xi  , 2, 1, 1);
        book(_h_sigm, 2, 1, 2);
        book(_h_sigp, 2, 1, 3);
      }
      else if (beamEnergyMatch(3.686*GeV)) { ///< @todo Was 1e-1 tolerance
        book(_h_xi ,  2, 1, 4);
        book(_h_sigm, 2, 1, 5);
        book(_h_sigp, 2, 1, 6);
      }
    ...
    void finalize() {
      // find energy
      int ioff=-1;
      if(fuzzyEquals(sqrtS(), 3.1*GeV, 1e-1)) {
        ioff=0;
      }
      else if (fuzzyEquals(sqrtS(), 3.686*GeV, 1e-1)) {
        ioff=1;
      }
