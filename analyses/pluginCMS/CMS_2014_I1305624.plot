# BEGIN PLOT /CMS_2014_I1305624/d01-x01-y01
Title=Transverse thrust for $110 < p_\mathrm{T,1} < 170 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\tau_{{\perp^}})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\tau_{_{\perp^}})$
LogY=0
#XLabelSep=4.5
#YLabelSep=4.5
LegendXPos=0.20
LegendYPos=0.70
#RatioPlotMode=datamc
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x01-y02
Title=Transverse thrust for $170 < p_\mathrm{T,1} < 250 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\tau_{{\perp^}})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\tau_{_{\perp^}})$
LogY=0
LegendXPos=0.20
LegendYPos=0.70
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x01-y03
Title=Transverse thrust for $250 < p_\mathrm{T,1} < 320 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\tau_{{\perp^}})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\tau_{_{\perp^}})$
LogY=0
LegendXPos=0.20
LegendYPos=0.70
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x01-y04
Title=Transverse thrust for $320 < p_\mathrm{T,1} < 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\tau_{{\perp^}})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\tau_{_{\perp^}})$
LogY=0
LegendXPos=0.20
LegendYPos=0.70
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x01-y05
Title=Transverse thrust for $p_\mathrm{T,1} > 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\tau_{{\perp^}})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\tau_{_{\perp^}})$
LogY=0
LegendXPos=0.20
LegendYPos=0.70
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x02-y01
Title=Jet broadening for $110 < p_\mathrm{T,1} < 170 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(B_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(B_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x02-y02
Title=Jet broadening for $170 < p_\mathrm{T,1} < 250 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(B_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(B_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x02-y03
Title=Jet broadening for $250 < p_\mathrm{T,1} < 320 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(B_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(B_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x02-y04
Title=Jet broadening for $320 < p_\mathrm{T,1} < 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(B_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(B_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x02-y05
Title=Jet broadening for $p_\mathrm{T,1} > 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(B_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(B_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x03-y01
Title=Total jet mass for $110 < p_\mathrm{T,1} < 170 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x03-y02
Title=Total jet mass for $170 < p_\mathrm{T,1} < 250 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x03-y03
Title=Total jet mass for $250 < p_\mathrm{T,1} < 320 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x03-y04
Title=Total jet mass for $320 < p_\mathrm{T,1} < 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x03-y05
Title=Total jet mass for $p_\mathrm{T,1} > 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x04-y01
Title=Total transverse jet mass for $110 < p_\mathrm{T,1} < 170 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho^\mathrm{T}_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho^\mathrm{T}_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x04-y02
Title=Total transverse jet mass for $170 < p_\mathrm{T,1} < 250 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho^\mathrm{T}_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho^\mathrm{T}_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x04-y03
Title=Total transverse jet mass for $250 < p_\mathrm{T,1} < 320 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho^\mathrm{T}_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho^\mathrm{T}_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x04-y04
Title=Total transverse jet mass for $320 < p_\mathrm{T,1} < 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho^\mathrm{T}_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho^\mathrm{T}_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x04-y05
Title=Total transverse jet mass for $p_\mathrm{T,1} > 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(\rho^\mathrm{T}_\mathrm{tot})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(\rho^\mathrm{T}_\mathrm{tot})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT


# BEGIN PLOT /CMS_2014_I1305624/d01-x05-y01
Title=Third-jet resolution parameter for $110 < p_\mathrm{T,1} < 170 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(Y_{ 23})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(Y_{ 23})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x05-y02
Title=Third-jet resolution parameter for $170 < p_\mathrm{T,1} < 250 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(Y_{ 23})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(Y_{ 23})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x05-y03
Title=Third-jet resolution parameter for $250 < p_\mathrm{T,1} < 320 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(Y_{ 23})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(Y_{ 23})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x05-y04
Title=Third-jet resolution parameter for $320 < p_\mathrm{T,1} < 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(Y_{ 23})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(Y_{ 23})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT

# BEGIN PLOT /CMS_2014_I1305624/d01-x05-y05
Title=Third-jet resolution parameter for $p_\mathrm{T,1} > 390 \,\, \mathrm{and} \mid\eta\mid < 2.4$
XLabel=$\ln(Y_{ 23})$
YLabel=$1/N \,\, \mathrm{d}N/\mathrm{d}\,\ln(Y_{ 23})$
LogY=0
LegendXPos=0.4
LegendYPos=0.30
# END PLOT
