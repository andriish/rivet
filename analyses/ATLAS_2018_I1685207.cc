    ...
    void analyze(const Event& event) {
     ...
     // }
      ...
      for (const PseudoJet& j : signal){ 
        ...
        // checking for V or top in the event & getting the deltaR
        ...
        // the three lines below are original code used to tag jets 
    
        // MCBot_TagType mcbottag = _MCbottagger->tag(j, tagged_constituents);
        // VRCjet_tags.push_back(static_cast<DNN_Category>(mcbottag));
        // counter++;
      ...
      if (smeared_small_jets[0].pT() <= 300*GeV || smeared_small_jets[1].pT() <= 200*GeV ||
        smeared_small_jets[0].pT() <= 125*GeV || smeared_small_jets[1].pT() <= 75*GeV){
        vetoEvent;
      }
      //2 bjets
