...
namespace Rivet {
  ...
  public:
    ...
    void analyze(const Event& event) {
      ...
      vector<Jet> cutJetsDIS;
      vector<Jet> cutJetsPHO;
      ...
      if (isDIS) {
        ...
        for(std::vector<int>::size_type i=0; i<DISjets.size(); i++){
        ...
        for(std::vector<int>::size_type i=0; i<PHOjets.size(); i++){
      ...
      const Particles PHO_muons = filter_select(all_muons, [](const Particle& m) {
            return m.eta() > -0.55 && m.eta() < 1.1 && m.pT() > 2.5*GeV; 
      });
      ...
      if (isDIS ) {
        ...
        //log10 check c++ formatting.
	
      } 
      else {
	
	      // cout << " Photo muons " << PHO_muons.size() << endl;
        for (const Particle& m : PHO_muons) {
           //cout << " photon muon id: " << m.pid() << endl;
